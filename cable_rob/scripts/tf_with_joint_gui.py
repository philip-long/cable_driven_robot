...
#    master.maxsize(width=24*SCALE, height=18*SCALE)
    ...
    def __init__(self):
        ...
        initial_value=[0.402,0.3246,0.4746,0.0,0.0,0.0]
        for i in range(8):
            ...
            if(i<6): # Create the tf functions
                self.wTp_inputs.append(self.make_entry_button_for_transform(self.dof[i],initial_value[i]))
...
#        self.w=Label(self.master,textvariable=self.v,bg="white").grid(row=0,column=6)
    ...
    def make_entry_button_for_transform(self,name,val):
